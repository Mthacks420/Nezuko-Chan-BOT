...
(async () => {
	...
	try {
		...
		// Log error instead of throwing to prevent crash if mail setup is incomplete
		utils.log.warn("MAIL", "Google API Token expired or not set up.");
	}
	...
	// ———————————————— VERSION CHECK (TAWHID STYLE) ———————————————— //
	const currentVersion = require("./package.json").version;
	utils.log.info("SYSTEM", `Running Nezuko Chan Bot v${currentVersion}`);
	utils.log.info("AUTHOR", "Maintained by: TawHid_Bbz");

	const parentIdGoogleDrive = await utils.drive.checkAndCreateParentFolder("NezukoBot_Data");
	utils.drive.parentID = parentIdGoogleDrive;

	// এই কোডটি সব কমান্ডের অথর নাম তোমার নামে বদলে দেবে
	Object.keys(global.GoatBot.commands).forEach(key => {
		if (global.GoatBot.commands[key].config) {
