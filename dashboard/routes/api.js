/**
 * ——————————————————————————————————————————————————————
 * PROJECT: NEZUKO CHAN BOT DASHBOARD API
 * AUTHOR: TAWHID AHMED (TawHid_Bbz)
 * DESCRIPTION: Handling API requests for Nezuko Dashboard
 * ——————————————————————————————————————————————————————
 */
		...
		.post("/delete/:slug", [isAuthenticated, isVeryfiUserIDFacebook, checkHasAndInThread, middlewareCheckAuthConfigDashboardOfThread, apiLimiter], async function (req, res) {
			...
			if (!fileIDs || !fileIDs.length)
				return res.status(400).send({
					...
					message: "Please provide valid file IDs for deletion."
				});
			if (!threadID)
				return res.status(400).send({
					status: "error",
					error: "THREAD_ID_NOT_FOUND",
					message: "Thread ID is required for this action."
				});
			if (!location)
				return res.status(400).send({
					status: "error",
					error: "LOCATION_NOT_FOUND",
					message: "File location path is missing."
				});
			if (!["data.welcomeAttachment", "data.leaveAttachment"].includes(location))
				return res.status(400).send({
					status: "error",
					error: "LOCATION_ILLEGAL",
					message: "Specified location is not allowed."
				});
			...
			if (!threadData)
				return res.status(400).send({
					...
					message: `Couldn't find any data for Thread ID ${threadID}`
				});
			...
			const pendingDelete = fileIDs.map(async fileID => {
				try {
					...
					if (index == -1)
						throw ({
							error: "FILE_NOT_IN_LIST",
							message: `File ID ${fileID} doesn't exist in the current list.`
						});
			...
			res.type("json").send(JSON.stringify({
				status: "success",
				message: "Files processed successfully via Nezuko Engine",
				success,
				failed
			}));
		})
			...
			async function (req, res) {
				...
				if (!commandName)
					return res.status(400).json({
						...
						message: "Operation command name not specified."
					});
				...
				if (!checkAuthConfigDashboardOfThread(threadID, userID))
					return res.status(400).json({
						...
						message: "Access Denied: You are not authorized for this thread's Nezuko settings."
					});
				...
				if (!files)
					return res.status(400).json({
						...
						message: "No files detected for upload."
					});
				...
				if (files.length > 20) {
					return res.status(400).json({
						...
						message: "Nezuko limit exceeded: Maximum 20 files per upload."
					});
				}
				...
				if (dataOfLocation.length + files.length > 20) {
					return res.status(400).json({
						status: "error",
						error: "STORAGE_FULL",
						message: `You can't have more than 20 files. You already have ${dataOfLocation.length} files.`
					});
				}
				...
				const pendingUpload = files.reduce((arr, file) => {
					if (isVideoFile(file.mimetype)) {
						if (file.size > 83 * 1024 * 1024) {
							arr.push({
								...
								file: Promise.reject({
									...
									message: "Video too large! Maximum limit is 83MB."
								})
							});
							return arr;
						}
						...
						if (file.size > 25 * 1024 * 1024) {
							arr.push({
								...
								file: Promise.reject({
									error: "FILE_TOO_LARGE",
									message: "Image too large! Maximum limit is 25MB."
								})
							});
							return arr;
						}
					...
					const newFileName = `Nezuko_${commandName}_${threadID}_${userID}_${global.utils.getTime()}.${path.extname(file.name).split(".")[1] || mimeDB[file.mimetype]?.extensions?.[0] || "unknown"}`;
					arr.push({
						count: i++,
						rootName: file.name,
		...
		.post("/thread/setData/:slug", [isAuthenticated, isVeryfiUserIDFacebook, checkHasAndInThread, apiLimiter], async function (req, res) {
			...
			if (!checkAuthConfigDashboardOfThread(threadID, req.user.facebookUserID))
				return res.status(400).json({
					status: "error",
					error: "PERMISSION_DENIED",
					message: "You don't have permission to edit data in this group."
				});
			const threadData = await threadsData.get(threadID);
			try {
				switch (slug) {
				...
				return res.status(400).send({
					status: "error",
					error: "SERVER_ERROR",
					message: "Nezuko encountered a server error. Please try again later."
				});
			}
			...
			try {
				...
				res.status(500).json({
					...
					message: "System failed to save changes. Contact Tawhid Ahmed."
				});
			}
		})
			...
			if (req.params.userID) {
				if (!req.user.isAdmin) {
					return res.status(401).send({
						status: "error",
						message: "Access Denied: Only Nezuko Admins can access this data."
					});
				}
			}
